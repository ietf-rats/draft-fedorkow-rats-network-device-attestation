...
stand_alone: true
ipr: trust200902
docname: draft-fedorkow-rats-network-device-attestation-03
cat: info
pi:
  toc: 'yes'
  tocdepth: '4'
...
is based on the standard roles defined in {{I-D.ietf-rats-architecture}}:
...
* In Step 2, measured values are retrieved from the Attester's TPM using a
  ...
  for Basic Challenge-Response-based Remote Attestation Procedures {{-rats-charra}}).
...
5. Quotes are retrieved according to TCG TAP Information Model {{TAP}}.  While the TAP IM gives a protocol-independent description of the data elements involved, it's important to note that quotes from the TPM are signed inside the TPM, so must be retrieved in a way that does not invalidate the signature, as specified in {{-rats-charra}}, to preserve the trust model.  (See Security Considerations).
